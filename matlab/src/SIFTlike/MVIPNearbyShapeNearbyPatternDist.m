function [ Dist ] = MVIPNearbyShapeNearbyPatternDist( query,dataset,PIPthr )
...
[IndicatorQ,~] = getIndicator_nearbyshapenearbypattern( query, PIPinfoQ );%query's info

IndicatorD=cell(rnum,1);%dataset's info
for i=1:rnum
    [ tmp,~ ] = getIndicator_nearbyshapenearbypattern( dataset(i,:), PIPinfoD{i,1} );
    IndicatorD{i,1}=tmp;
end
